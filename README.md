...
```bash
stock-trader-app$ sam build --use-container
```
...
```bash
stock-trader-app$ sam logs -n StockCheckerFunction --stack-name stock-trader-app --tail
```
...
```bash
stock-trader-app$ pip install -r tests/requirements.txt --user
# unit test
stock-trader-app$ python -m pytest tests/unit -v
# integration test, requiring deploying the stack first.
# Create the env variable AWS_SAM_STACK_NAME with the name of the stack we are testing
stock-trader-app$ AWS_SAM_STACK_NAME=<stack-name> python -m pytest tests/integration -v
```
...
```bash
aws cloudformation delete-stack --stack-name stock-trader-app
```
